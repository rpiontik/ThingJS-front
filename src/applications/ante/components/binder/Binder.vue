...
export default {
    ...
    reqTree: {
      handler (reqTree) {
        ...
        for (let f = 0; result && (f < reqTree.length); f++) {
          let node = reqTree[f];
          if (!node.apply) { continue; }
          let pin = null;
          switch (node.struct_type) {
            case 'set': {
              pin = [];
              for (let i = 0; pin && (i < node.children.length); i++) {
                if (node.children[i].bind == null) { pin = null; } else { pin.push(node.children[i].bind); }
              }
              break;
            }
            case 'pin': {
  ...
  computed: {
    ...
    requiredTree () {
      if (this.reqTree) { return this.reqTree; }

      if (!this.manifest || !('requires' in this.manifest) || !('interfaces' in this.manifest.requires)) { return null; }
      ...
      for (let iName in this.manifest.requires.interfaces) {
        ...
        if (iData.type in availableInterfaces) {
          ...
          node.cases.map((item) => {
            if (item.length > maxPins) { maxPins = item.length; }
    ...
    searchCandidate (require) {
      ...
      for (let i = 0; i < cases.length; i++) {
        ...
        for (let index = 0; index < cases[i].length; index++) {
          ...
          if (!children[index].bind) { overlap += 0.005; } else if (enumPins.indexOf(children[index].bind) >= 0) { overlap += 1; }
        }
        if (overlap > 0) { candidate.valid_cases.push(cases[i]); }
