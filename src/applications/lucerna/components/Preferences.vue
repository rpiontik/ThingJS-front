...
export default {
    ...
    uploadLEDFile (evt, channel) {
      ...
      reader.onload = (event) => {
        try {
          ...
          if (error_format) { throw 'Error format file'; }
  ...
  data () {
    ...
    } if (!this.$store.state.Lucerna.data.config || !this.$store.state.Lucerna.data.config.length) { data.config = default_config; } else { data.config = this.copyConfig(this.$store.state.Lucerna.data.config[0], 'ui'); }

    if (!this.$store.state.Lucerna.data.spectrum) {
      this.$store.dispatch('Lucerna/data/reload', 'spectrum');
      data.spectrum = [];
    } else { data.spectrum = this.copySpectrum(this.$store.state.Lucerna.data.spectrum, 'ui'); }
