        ...
        <template v-for="channel, index in channels">
            <g :key="index">
                ...
                <line
                        ...
                        :y2="channel.y"
                        :style="{'stroke' : getContrastColor(channel.color.slice(-6))}"
                        @mousemove.stop.prevent="onMouseMove(channel, $event)"
                        @mousedown.stop.prevent
                />
                <circle
                        ...
                        :style="{'fill' : getContrastColor(channel.color.slice(-6))}"
  ...
  methods: {
    doCloseChannels (except) {
      this.channels.map((channel) => {
        if (channel !== except) { this.doExpandChannel(channel, false); }
      });
    },
    doExpandChannel (channel, value) {
      if (!channel.expand && value) { this.sliding = false; }
      if (value) { this.doCloseChannels(channel); }
    ...
    calcLevelsByBrightness () {
      ...
      for (let channel = 1; channel < this.channels.length; channel++) {
        if (this.channels[channel].level > max) { max = this.channels[channel].level; }
      }

      let brightness = this.channels[0].level;

      for (let channel = 1; channel < this.channels.length; channel++) { this.channels[channel].level = !max ? brightness : brightness * (this.channels[channel].level / max); }
    },
    ...
    onMouseMove (channel, event) {
      ...
      if (this.sliding) {
        ...
        if (channel.index === 0) { this.calcLevelsByBrightness(); } else { this.calcBrightnessByLevels(); }
    ...
    onMouseLeave (channel, event) {
      if (('toElement' in event) && (event.toElement.parentElement !== event.target.parentElement)) { this.doExpandChannel(channel, false); }
    },
    ...
    remakeParams (params) {
      if (!params.channels) { params.channels = []; }
      ...
      this.value.map((source, index) => {
        if (!params.channels[index]) { params.channels[index] = {}; }
        ...
        topOffset += (!topOffset ? params.radius * 0.75 : 0) + params.radius * 2 + 2;
      });
